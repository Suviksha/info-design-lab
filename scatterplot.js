function scatterplot(){
	...
	"</br>"+"Cash-GDP ratio:    <span style ='color:#fc8d59'>"+d.Cash+"  </span> USD"});
    ...
    var tip_2=d3.tip()
	            ...
	            .html(function(d){
	   return  "  <span style ='color:#ffeda0'>"+d.Country+"</span>"+"</br>"+"Per Capita GNI:    <span style ='color:#ff7f7f'>"+d.GNI+"  </span> USD "+ 
	          "</br>"+"Corruption Index:   <span style ='color:#91cf60'>"+d.cpi+"</span> USD"});
...
svg.call(tip_2);
  ...
  svg.append("g")			
      .attr("class", "grid")
      .call(makeGrid().innerTickSize(-width).tickFormat(" "))
	  .style("fill","none")
	  .style("stroke","gray")
  ...
  svg.append("g")
      ...
      .text("Per Capita GNI")
	  .call(d3.svg.axis().scale(x).orient("bottom").tickPadding(10).ticks(11))
	  .append("text")
      .attr("class", "label")
      .attr("x", width)
      .attr("y", -6)
	  ...
	  .style("opacity",1)
	  .style("fill","black")
      .text(" Per Capita GNI ($ USD) ");

  // Add the y-axis.
  svg.append("g")
      .attr("class", "y axis")
	  .text("Cash","CPI")
      .call(d3.svg.axis().scale(y).orient("left").tickPadding(10).ticks(11))
	  .append("text")
      .attr("class", "label1")
...
svg.selectAll("lines")
   ...
   .attr("y2",function(d){return y(+d.cpi)})
   .style("stroke","black")
   .style("stroke-width",0.5)
   .style("opacity",1);
...
svg.selectAll(".text1")
   ...
   .attr("id","text1")
   .attr("x",function(d){return x(+d["GNI"])+10;})
   .attr("y",function(d){return y(+d.cpi)+10;})
   .style("font_family","sans-serif")
   .style("font-size","8px")
   .style("color","black")
   .style("opacity",0)
...
form.append('input')
	...
	.on('click', function () {
		...
		d3.selectAll(".label1").transition().duration(1000).delay(250).ease("linear").style("opacity",0).remove();
		d3.selectAll("g.y.axis").append("text").attr("class","label1").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".50em")
		.style("text-anchor", "end").style("fill","black").text("Cash (in $USD)").style("opacity",0).transition().duration(2000).delay(250).ease("linear").style("opacity",1);
    });
...
form.append('input')
	...
	.on('click', function () {
		...
		d3.selectAll(".label1").transition().duration(1000).delay(250).ease("linear").style("opacity",0).remove();
		d3.selectAll("g.y.axis").append("text").attr("class","label1").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".50em")
		.style("text-anchor", "end").style("fill","black").text("Corruption").style("opacity",0).transition().duration(2000).delay(250).ease("linear").style("opacity",1);
